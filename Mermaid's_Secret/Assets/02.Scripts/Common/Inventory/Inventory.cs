...
public class Inventory : MonoBehaviour
{
    ...
    //다리 설치에 필요한 통나무 아이템을 가지고 있는지를 확인하는 파라미터(외부 참조)
    public bool canSetBridge
    {
        get
        {
            bool haveWood = false;
            for (int a = 0; a < m_S_slots.Length; a++)  //모든 슬롯을 체크할 동안 for문을 돌리고
            {
                if (m_S_slots[a].item != null && m_S_slots[a].item.itemName == "통나무")  //아이템이 들어있는 슬롯 중 통나무 아이템을 가진 슬롯이 있다면
                {
                    haveWood = true;
                    break;
                }
    ...
    //다리 설치 영역(조사하기 영역)에서 다리 설치 커맨드(E키) 입력 시 실행할 함수
    public void useWood()
    {
        for (int a = 0; a < m_S_slots.Length; a++)  //모든 슬롯을 체크할 동안 for문을 돌리고
        {
            if (m_S_slots[a].item != null && m_S_slots[a].item.itemName == "통나무")  //아이템이 들어있는 슬롯 중 통나무를 가진 슬롯을 찾아서
            {
                m_S_slots[a].ClearSlot();   //슬롯 비워주기
                break;
            }
        }
    }
